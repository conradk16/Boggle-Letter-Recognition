...
class Network:
    ...
    def SGD(self, training_data, mini_batch_size, eta, validation_data=None, regularization_param = 0.0, epochs_to_decrease_eta=2, eta_decrease_fraction=0.5, num_eta_decreases=5):
        n = len(training_data)
        ...
        while eta_decrease_count <= num_eta_decreases:
            ...
            print("epoch {0}: {1} / {2} - {3}%".format(j, num_correct, len(validation_data), num_correct / len(validation_data)))
            ...
            with open("saved_network.p", "wb") as f:
                pickle.dump(self, f)
